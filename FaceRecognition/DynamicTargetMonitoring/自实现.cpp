...
int main1(int argc, const char* argv[])
{
	...
	String str = "WavingTrees/b00";//string,the 's' can be a captial(资本) or lower-caseletters
	...
	//covert（转换） the colorful image to the corresponding gray-level image
	...
	//c++版本的opencv用Size函数即可，c版本的用cvSize()函数
	VideoWriter output_src("src.avi", CV_FOURCC('P', 'I', 'M', '1'), 20, Size(160, 120), 1);
	//VideoWriter output_src("src.avi",CV_FOURCC('M','J','P','G'),5,Size(160,120),1);//c++版本的opencv用Size函数即可，c版本的用cvSize()函数
	//这样输出的是3个通道的数据
	VideoWriter output_dst("dst.avi", CV_FOURCC('P', 'I', 'M', '1'), 20, Size(160, 120), 1);
	while (1)
	{
		if (!pause)
		{
			...
			i++;
			//the latest name is _itoa_s or _itoa,not the itoa,although iota can be used,deprecated
			_itoa_s(i, str_num, 10);
			if (i < 10)
				str += "00";
			else if (i < 100)
				str += "0";
	...
	for (int m = 0; m < img.rows; m++)
		for (int n = 0; n < img.cols; n++)
		{
			...
			for (; k < GMM_MAX_COMPONT; k++)
			{
				...
				if (dis < 3.0*sigma[k].at<float>(m, n))//the present pixpel is fit the component(成分)
				{
					/****update the weight****/
