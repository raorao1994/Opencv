...
Mat read_mnist_label(const string fileName) {
	...
	if (file.is_open())
	{
		...
		cout << "开始读取Label数据......\n";
		start_time = clock();
		LabelMat = Mat::zeros(number_of_items, 1, CV_32SC1);//CV_32SC1
		for (int i = 0; i < number_of_items; i++) {
			unsigned char temp = 0;
			file.read((char*)&temp, sizeof(temp));
			unsigned int _temp= (unsigned int)temp;
			LabelMat.at<int>(i, 0) = _temp;
			//unsigned int _tempq = LabelMat.at<unsigned int>(i, 0);
			//打印第一个和最后一个label
